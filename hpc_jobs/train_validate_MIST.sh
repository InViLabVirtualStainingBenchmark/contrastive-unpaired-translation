#!/bin/bash
#SBATCH --job-name=cut_train_validate_MIST
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH -p ampere_gpu
#SBATCH --gres=gpu:1
#SBATCH -o /data/antwerpen/212/vsc21212/projects/cut/logs/train_validate_MIST.%j.out
#SBATCH -e /data/antwerpen/212/vsc21212/projects/cut/logs/train_validate_MIST.%j.err

# train_validate_MIST.sh
# Runs 2 epochs of CUT training on MIST-HER2 as a cluster confirmation gate.
# This job must pass before submitting the full MIST-HER2 training jobs.
#
...
#        find $VSC_DATA/projects/cut/outputs/checkpoints/MIST_smoke_e2 -name "*.pth"
#   4. GPU log CSV has entries:
#        tail -5 $VSC_DATA/projects/cut/logs/gpu_train_validate_MIST.csv
...
nvidia-smi --query-gpu=timestamp,utilization.gpu,memory.used,memory.total \
           --format=csv -l 5 \
    > "$VSC_DATA/projects/cut/logs/gpu_train_validate_MIST.csv" & GPU_LOG_PID=$!
...
echo ""
echo "=== Starting MIST-HER2 validation training (2 epochs, load 1024, crop 512) ==="
...
echo ""
echo "=== GPU log tail ==="
tail -3 "$VSC_DATA/projects/cut/logs/gpu_train_validate_MIST.csv"
